...
app.get('/api/daily-tip', async (req, res) => { // Made async
    try {
        const { category, random, limit = 8, page = 1 } = req.query;
        ...
        const [countResults] = await req.db.execute(countQuery, countParams); // Use req.db.execute()
        const totalTips = countResults[0].total;
        const totalPages = Math.ceil(totalTips / limitNum);

        // If requesting random tips, handle differently
        if (random === 'true') {
            let randomQuery = 'SELECT * FROM daily_tips';
            let randomParams = [];

            if (category) {
                randomQuery += ' WHERE category = ?';
                randomParams.push(category);
            }

            randomQuery += ' ORDER BY RAND() LIMIT ?';
            randomParams.push(limitNum);

            const [results] = await req.db.execute(randomQuery, randomParams); // Use req.db.execute()
        ...
        const [results] = await req.db.execute(query, queryParams); // Use req.db.execute()
...
app.post('/api/self-assessment', async (req, res) => { // Made async
    try {
        ...
        const python = spawn('python', ['predict_model.py', dataString]);
